...
openssl genrsa -out bob.priv 2048 && openssl rsa -in bob.priv -outform PEM -pubout -out bob.pub
 ...
 gcc -std=c99 -g -I/usr/local/openssl/include test3.c -o test3 -L/usr/local/openssl/lib -lssl -lcrypto -ldl -fno-stack-protector

gcc -c socks.c -o socks.o && gcc -std=c99 -g -I/usr/local/openssl/include test5.c socks.c -o server -L/usr/local/openssl/lib -lssl -lcrypto -ldl -fno-stack-protector
*/
