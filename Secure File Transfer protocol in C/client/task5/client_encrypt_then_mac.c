...
int main(int argc , char *argv[])
{
	...
	if (strstr(server_reply,"Incominglength=")!=NULL)
	{
		...
		char *token = strtok(cop,"$");//incominglength 
		char *token2 = strtok(NULL,"$");//=length of cipher
		...
		//while(recv(sock, server_reply, 2000 , 0)){
			...
			char *token4=strtok(NULL,"$");//=encrypted cipher
			//token3=strtok(NULL,"Message begins:");
			
			strcpy(decryptedmsg,decryptMsg(token4,ciphertext_len));
			char *token5=strtok(NULL,"$");//HMAC:
			char token6[40];//=hmac
			strcpy(token6,strtok(NULL,"$"));
			if(verifyHmac(token4,token6))
				{puts("Hmac verified");}
		//}
